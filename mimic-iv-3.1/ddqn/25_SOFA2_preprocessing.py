...
def build_states(con, base, out_state, bin_hours):
...
def main():
    ...
    ap.add_argument("--mem_limit", default="8GB")
    ap.add_argument("--tmp", default="./duckdb_tmp")
    args = ap.parse_args()
    ...
    tmp_state = out.with_name(out.stem + "_states_tmp.parquet")
    ...
    print(f"[bc-prep] OUT={out}")
    print(f"[bc-prep] building states -> {tmp_state}")
    build_states(con, base, tmp_state, args.bin_hours)

    print("[bc-prep] building sepsis cohort transitions")
    build_transitions(con, base, tmp_state, out, args.bin_hours)
