#!/usr/bin/env python3
"""
Lite MIMIC-IV preprocessing for BCQ using DuckDB.

Outputs a transitions parquet with columns:
  stay_id, bin, action, reward, done,
  vaso_amount, fluid_amount, sofa_total,
  s_*, s_next_*

Action space is 25 discrete treatment combinations:
  action = vaso_bin * 5 + fluid_bin
where both vaso_bin and fluid_bin are in {0,1,2,3,4}.

States are 4h-bin averages from chartevents plus demographics.
Rewards are terminal only by default:
  death    -> -100
  survival -> +100
  nonterminal -> 0
"""
...
def build_states(con: duckdb.DuckDBPyConnection, base: Path, out_state: Path, bin_hours: int) -> None:
...
def build_transitions(con: duckdb.DuckDBPyConnection, base: Path, state_file: Path, out_file: Path, bin_hours: int) -> None:
    ...
    WHERE stay_id IS NOT NULL AND subject_id IS NOT NULL AND hadm_id IS NOT NULL AND intime IS NOT NULL;
    """)
    ...
    SELECT subject_id::BIGINT AS subject_id,
           ...
           CASE WHEN gender = 'M' THEN 1.0 WHEN gender = 'F' THEN 0.0 ELSE NULL END::DOUBLE AS sex
    ...
    WHERE stay_id IS NOT NULL AND starttime IS NOT NULL AND itemid IN ({all_items});
    """)
    ...
    WHERE hadm_id IS NOT NULL AND charttime IS NOT NULL AND valuenum IS NOT NULL
    ...
    WHERE hadm_id IS NOT NULL AND starttime IS NOT NULL
      AND (LOWER(drug) LIKE '%cillin%' OR LOWER(drug) LIKE '%cef%' OR LOWER(drug) LIKE '%mycin%'
        OR LOWER(drug) LIKE '%penem%' OR LOWER(drug) LIKE '%floxacin%'
        OR LOWER(drug) LIKE '%metronidazole%' OR LOWER(drug) LIKE '%vancomycin%');
    """)
    ...
    SELECT DISTINCT abx.hadm_id, LEAST(abx.abx_time, cx.cx_time) AS inf_time
    FROM abx JOIN cx USING (hadm_id)
    WHERE (cx.cx_time <= abx.abx_time AND abx.abx_time <= cx.cx_time + INTERVAL '72 hours')
       OR (abx.abx_time < cx.cx_time AND cx.cx_time <= abx.abx_time + INTERVAL '24 hours');
    """)
    ...
    con.execute(f"""
    CREATE TABLE tr AS
    SELECT s.stay_id, s.bin,
           COALESCE(MAX(CASE WHEN ie.itemid IN ({vaso_sql})
                              AND ie.rate IS NOT NULL
                              AND ie.patientweight IS NOT NULL
                              AND ie.patientweight > 0
                             THEN ie.rate / ie.patientweight ELSE NULL END), 0.0) AS vaso,
           COALESCE(SUM(CASE WHEN ie.itemid IN ({fluid_sql}) THEN COALESCE(ie.amount, 0.0) ELSE 0.0 END), 0.0) AS fluid
    FROM ns s
    ...
    SELECT s.stay_id, s.bin,
      (
        CASE WHEN COALESCE(tr.vaso, 0) > 0.1 THEN 4 WHEN COALESCE(tr.vaso, 0) > 0 THEN 3 WHEN s.MBP < 70 THEN 1 ELSE 0 END
        + CASE WHEN lb.creatinine > 5 THEN 4 WHEN lb.creatinine > 3.5 THEN 3 WHEN lb.creatinine > 2 THEN 2 WHEN lb.creatinine > 1.2 THEN 1 ELSE 0 END
        + CASE WHEN lb.platelets < 20 THEN 4 WHEN lb.platelets < 50 THEN 3 WHEN lb.platelets < 100 THEN 2 WHEN lb.platelets < 150 THEN 1 ELSE 0 END
        + CASE WHEN lb.bilirubin > 12 THEN 4 WHEN lb.bilirubin > 6 THEN 3 WHEN lb.bilirubin > 2 THEN 2 WHEN lb.bilirubin > 1.2 THEN 1 ELSE 0 END
    ...
    SELECT
      ...
      s.bin,
      (
        CASE WHEN COALESCE(tr.vaso, 0) <= 0 THEN 0
        ...
        * 5
        +
        CASE WHEN COALESCE(tr.fluid, 0) <= 0 THEN 0
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--base", type=str, required=True, help="Path to mimic-iv-3.1 directory")
    p.add_argument("--out", type=str, default="Processed/transitions.parquet", help="Output transitions parquet path")
    p.add_argument("--bin_hours", type=int, default=4)
    p.add_argument("--threads", type=int, default=8)
    p.add_argument("--mem_limit", type=str, default="8GB")
    p.add_argument("--tmp", type=str, default="./duckdb_tmp")
    return p.parse_args()
...
def main() -> None:
    ...
    print(f"[bcq-processing] BASE={base}")
    print(f"[bcq-processing] building states -> {out_state}")
    build_states(con, base, out_state, args.bin_hours)
    print(f"[bcq-processing] building transitions -> {out_file}")
    build_transitions(con, base, out_state, out_file, args.bin_hours)
    print("[bcq-processing] Done.")
